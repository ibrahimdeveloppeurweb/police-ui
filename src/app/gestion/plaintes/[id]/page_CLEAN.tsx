...
import {
  ...
  AlertTriangle, CheckCircle, Clock, Phone, Mail, Loader2, Save, X, Users, Eye, Search, Lock, Plus
} from 'lucide-react'
...
import { PreuvesList, ActesEnqueteList, TimelineInvestigation } from '@/components/plaintes'
...
export default function PlainteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleOpenAssignModal = async () => {
    ...
    // Récupérer l'ID du commissariat depuis la plainte
    if (!plainte?.commissariat_id) {
      console.error('Commissariat ID manquant dans la plainte')
      alert('Erreur: Impossible de charger les agents (commissariat non défini)')
      return
    }
    
    try {
      console.log(`🔍 Chargement des agents du commissariat: ${plainte.commissariat_id}`)
      const response = await api.get(`/commissariat/${plainte.commissariat_id}/agents`)
      console.log('✅ Agents chargés:', response.data)
  ...
  return (
    <div className="min-h-screen space-y-6">
          ...
          <CardBody className="p-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="font-bold text-slate-900">{plainte.nombre_convocations}</div>
              </div>
            </div>
      ...
      <Modal isOpen={showEtapeModal} onClose={() => setShowEtapeModal(false)}>
        ...
        <ModalFooter>
          <Button onClick={() => setShowEtapeModal(false)}>
            Annuler
          </Button>
          <Button 
            onClick={handleChangerEtape}
            disabled={!nouvelleEtape}
            className="bg-blue-600 hover:bg-blue-700 text-white"
          >
            Confirmer
          </Button>
