...
import {
  PackageCheck, Search, Calendar, Eye, Printer, AlertTriangle,
  CheckCircle, Clock, TrendingUp, TrendingDown, FileDown, MapPin,
  User, Archive, Phone, Mail, Plus, X, PackageSearch, FileText, Loader2
} from 'lucide-react'
...
import { useObjetsRetrouvesWithFilters, type ObjetRetrouveListItem, type ObjetRetrouveStats } from '@/hooks/useObjetsRetrouves'
...
export default function ObjetsRetrouvesGestionListePage() {
  ...
  useEffect(() => {
    ...
    setTitle("Liste des Objets Retrouvés")
  ...
  const currentData = {
    ...
    stats: stats || {
      totalObjets: 0,
      disponibles: 0,  // ✅ CORRIGÉ: pluriel
      restitues: 0,
      nonReclames: 0,
      tauxRestitution: 0,
      evolutionTotal: "0",
      evolutionDisponibles: "0",  // ✅ CORRIGÉ: pluriel
  ...
  const handlePageChange = (newPage: number) => {
    console.log(`🔄 handlePageChange objets retrouvés: page actuelle = ${currentPage}, nouvelle page = ${newPage}`);
    setCurrentPage(newPage);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }

  const handleObjetRetrouveSubmit = (data: any) => {
    console.log('Nouvel objet retrouvé créé:', data)
    refetch()
    setIsModalOpen(false)
  }

  // Liste complète de tous les types d'objets (identique à objets perdus)
  const typesObjets = [
  ...
  // Extraire les types uniques des objets pour le filtre
  ...
  const getStatutColor = (statut: string) => {
    switch (statut) {
      ...
      case 'RESTITUÉ': return 'bg-green-500 text-white'
      case 'NON_RÉCLAMÉ': return 'bg-amber-500 text-white'
  ...
  const getStatutLabel = (statut: string) => {
    switch (statut) {
      ...
      case 'RESTITUÉ': return 'RESTITUÉ'
      case 'NON_RÉCLAMÉ': return 'NON RÉCLAMÉ'
    ...
    <div className="min-h-screen space-y-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Liste des Objets Retrouvés</h1>
        ...
        <p className="text-slate-600">Gestion des dépôts d'objets retrouvés</p>
      </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                  ...
                  className={`${globalFilter === 'jour' && !isCustomDateRange ? 'bg-emerald-600 text-white' : 'bg-white text-gray-700 border border-gray-300'} hover:bg-emerald-700 hover:text-white transition-colors text-xs md:text-sm px-3 py-2`}
                  >
                  Aujourd'hui
                  </Button>
                <Button 
                  onClick={() => handleFilterChange('semaine')}
                  className={`${globalFilter === 'semaine' && !isCustomDateRange ? 'bg-emerald-600 text-white' : 'bg-white text-gray-700 border border-gray-300'} hover:bg-emerald-700 hover:text-white transition-colors text-xs md:text-sm px-3 py-2`}
                >
                  Semaine
                </Button>
                <Button 
                  onClick={() => handleFilterChange('mois')}
                  className={`${globalFilter === 'mois' && !isCustomDateRange ? 'bg-emerald-600 text-white' : 'bg-white text-gray-700 border border-gray-300'} hover:bg-emerald-700 hover:text-white transition-colors text-xs md:text-sm px-3 py-2`}
                >
                  Mois
                </Button>
                <Button 
                  onClick={() => handleFilterChange('annee')}
                  className={`${globalFilter === 'annee' && !isCustomDateRange ? 'bg-emerald-600 text-white' : 'bg-white text-gray-700 border border-gray-300'} hover:bg-emerald-700 hover:text-white transition-colors text-xs md:text-sm px-3 py-2`}
                >
                  Année
                </Button>
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 pt-4 border-t border-emerald-200">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 flex-1 min-w-[200px]">
                ...
                <div className="flex items-center gap-2 w-full">
                  <input 
                    type="text"
                    placeholder="Rechercher par numéro, description, lieu, trouveur..."
                    value={searchTerm}
                    ...
                    className="border border-gray-300 rounded-lg px-3 py-2 text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 flex-1"
                  />
                  {searchTerm && (
              ...
              <select 
                ...
                className="px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 text-sm bg-white"
              >
                <option>Tous les statuts</option>
                <option>DISPONIBLE</option>
                <option>RESTITUÉ</option>
                <option>NON_RÉCLAMÉ</option>
              </select>
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 pt-4 border-t border-emerald-200">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <input 
                  ...
                  className="border border-gray-300 rounded-lg px-3 py-2 text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 w-full sm:w-auto"
                />
              </div>
              ...
              <div className="flex gap-2">
                ...
                <Button 
                  onClick={handlePrint}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs md:text-sm px-4 py-2"
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card className="bg-white border border-gray-200 border-t-4 border-t-blue-500 hover:shadow-lg transition-shadow">
          <CardBody className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-slate-600 text-sm font-medium uppercase">Disponible</h3>
              <div className="w-12 h-12 bg-blue-100 rounded-xl flex items-center justify-center">
                <Clock className="w-6 h-6 text-blue-600" />
              </div>
            </div>
            <div className="text-3xl font-bold text-slate-900 mb-2">{formatNumber(currentData.stats.disponibles)}</div>
            <div className={`flex items-center gap-2 text-sm font-medium ${
              currentData.stats.evolutionDisponibles 
                ? (currentData.stats.evolutionDisponibles.startsWith('-') 
                    ? 'text-red-600' 
                    : currentData.stats.evolutionDisponibles.startsWith('+') && currentData.stats.evolutionDisponibles !== '+0'
                      ? 'text-green-600'
                      : 'text-gray-600')
                : 'text-gray-600'
            }`}>
              {currentData.stats.evolutionDisponibles && currentData.stats.evolutionDisponibles !== '+0' && currentData.stats.evolutionDisponibles !== '0' ? (
                currentData.stats.evolutionDisponibles.startsWith('-') 
                  ? <TrendingDown className="w-4 h-4" />
                  : <TrendingUp className="w-4 h-4" />
              ) : (
              ...
              {currentData.stats.evolutionDisponibles || '0'} {getPeriodeLabel()}
            </div>
          </CardBody>
        </Card>
        ...
        <Card className="bg-white border border-gray-200 border-t-4 border-t-green-500 hover:shadow-lg transition-shadow">
          <CardBody className="p-6">
            ...
            <div className="text-3xl font-bold text-slate-900 mb-2">{formatNumber(currentData.stats.restitues)}</div>
            <div className={`flex items-center gap-2 text-sm font-medium ${
              currentData.stats.evolutionRestitues 
                ? (currentData.stats.evolutionRestitues.startsWith('-') 
                    ? 'text-red-600' 
                    : currentData.stats.evolutionRestitues.startsWith('+') && currentData.stats.evolutionRestitues !== '+0'
                      ? 'text-green-600'
                      : 'text-gray-600')
                : 'text-gray-600'
            }`}>
              {currentData.stats.evolutionRestitues && currentData.stats.evolutionRestitues !== '+0' && currentData.stats.evolutionRestitues !== '0' ? (
                currentData.stats.evolutionRestitues.startsWith('-') 
                  ? <TrendingDown className="w-4 h-4" />
                  : <TrendingUp className="w-4 h-4" />
              ) : (
              ...
              {currentData.stats.evolutionRestitues || '0'} {getPeriodeLabel()}
            </div>
          </CardBody>
        </Card>
        ...
        <Card className="bg-white border border-gray-200 border-t-4 border-t-amber-500 hover:shadow-lg transition-shadow">
          <CardBody className="p-6">
            ...
            <div className="text-3xl font-bold text-slate-900 mb-2">{formatNumber(currentData.stats.nonReclames)}</div>
            <div className={`flex items-center gap-2 text-sm font-medium ${
              currentData.stats.evolutionNonReclames 
                ? (currentData.stats.evolutionNonReclames.startsWith('-') 
                    ? 'text-red-600' 
                    : currentData.stats.evolutionNonReclames.startsWith('+') && currentData.stats.evolutionNonReclames !== '+0'
                      ? 'text-green-600'
                      : 'text-gray-600')
                : 'text-gray-600'
            }`}>
              {currentData.stats.evolutionNonReclames && currentData.stats.evolutionNonReclames !== '+0' && currentData.stats.evolutionNonReclames !== '0' ? (
                currentData.stats.evolutionNonReclames.startsWith('-') 
                  ? <TrendingDown className="w-4 h-4" />
                  : <TrendingUp className="w-4 h-4" />
              ) : (
              ...
              {currentData.stats.evolutionNonReclames || '0'} {getPeriodeLabel()}
            </div>
          </CardBody>
        </Card>
      </div>
      ...
      {activeTab === 'cartes' ? (
        ...
        {loading && currentData.objets.length === 0 ? (
          <Card>
            <CardBody className="p-6">
              <div className="text-center py-8">
                ...
                <p className="mt-4 text-slate-600">Chargement des objets retrouvés...</p>
              </div>
            </CardBody>
          </Card>
          ...
          <Card>
            <CardBody className="p-6">
              <div className="text-center py-8">
                <PackageCheck className="w-12 h-12 text-slate-400 mx-auto mb-4" />
                <p className="text-slate-600 font-medium">Aucun objet retrouvé trouvé</p>
          ...
          currentData.objets.map((objet) => (
            <Card key={objet.id} className="bg-white border border-gray-200 hover:shadow-lg transition-shadow cursor-pointer" onClick={() => router.push(`/gestion/objets-retrouves/${objet.id}`)}>
              <CardBody className="p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-right">
                    <div className="text-sm text-slate-500 mb-1">Déposé le</div>
                    <div className="text-lg font-bold text-emerald-600">{objet.dateDepot}</div>
                  </div>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div>
                    <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Déposant</p>
                    <p className="font-bold text-slate-900">{objet.deposant.prenom} {objet.deposant.nom}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Téléphone</p>
                    <p className="font-bold text-slate-900">{objet.deposant.telephone}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Lieu de trouvaille</p>
                    <p className="font-bold text-slate-900">{objet.lieuTrouvaille}</p>
                  </div>
                  <div>
                ...
                <div className="flex gap-3 flex-wrap">
                  <Button 
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      router.push(`/gestion/objets-retrouves/${objet.id}`)
                    }}
                  >
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-gray-200">
                    ...
                    <th className="text-left py-4 px-4 font-bold text-gray-700 uppercase text-sm">Trouveur</th>
                  ...
                  {loading && currentData.objets.length === 0 ? (
                    ...
                    <tr>
                      <td colSpan={7} className="py-8 text-center text-slate-600">
                        <PackageCheck className="w-12 h-12 text-slate-400 mx-auto mb-4" />
                        <p className="font-medium">Aucun objet retrouvé trouvé</p>
                    ...
                    currentData.objets.map((objet) => (
                      ...
                      >
                        <td className="py-4 px-4 font-semibold text-emerald-600">{objet.numero}</td>
                        ...
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-sm">{objet.deposant.prenom} {objet.deposant.nom}</span>
                          </div>
                        </td>
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-1">
                            <MapPin className="w-4 h-4 text-gray-400" />
                            <span className="text-sm max-w-[200px] truncate">{objet.lieuTrouvaille}</span>
                          </div>
                        </td>
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-1">
                            <Calendar className="w-4 h-4 text-gray-400" />
                            <span className="text-sm">{formatDateOnly(objet.dateTrouvaille)}</span>
                          </div>
                        </td>
                        <td className="py-4 px-4">
          ...
          <CardBody className="p-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <p className="text-sm text-slate-600">
                  ...
                  sur <span className="font-medium">{formatNumber(total)}</span> objets retrouvés
              </p>
      ...
      <ObjetRetrouveFormPage 
        ...
        onSubmit={handleObjetRetrouveSubmit}
      />
