...
import {
  PackageSearch, ArrowLeft, Calendar, MapPin, User, Phone, Mail,
  Clock, CheckCircle, AlertTriangle, Printer, Edit, FileText, Loader2,
  Archive, Bell, PackageCheck, Eye, Layers, Box, ShoppingBag, Briefcase,
...
import { useObjetPerduDetail } from '@/hooks/useObjetPerduDetail'
...
export default function ObjetPerduDetailPage() {
  ...
  const { objet, loading, error, refetch } = useObjetPerduDetail(objetId)
  ...
  // Mettre à jour le titre et sous-titre quand l'objet, le commissariat ou la route change
  useEffect(() => {
    ...
    if (objet) {
      setTitle(`Objet Perdu - ${objet.numero}`)
      // Utiliser le commissariat de l'utilisateur connecté avec formatage
      ...
      setTitle(`Objet Perdu - ${objetId}`)
      // Utiliser le commissariat de l'utilisateur connecté avec formatage
      if (formattedCommissariat) {
        setSubtitle(`${formattedCommissariat} - Détails de la déclaration`)
      } else {
        setSubtitle("Détails de la déclaration")
      }
    }
    
    // Cleanup: réinitialiser les valeurs quand on quitte la page
    return () => {
      // Ne pas réinitialiser ici car la nouvelle page va définir ses propres valeurs
    }
  ...
  const [actionType, setActionType] = useState<'retrouve' | 'cloture' | null>(null)
  const [dateRetrouve, setDateRetrouve] = useState('')
  ...
  const normalizedStatut = useMemo(() => {
    ...
    if (statut.includes('RECHERCHE')) return 'EN_RECHERCHE'
    if (statut.includes('RETROUV')) return 'RETROUVE'
    if (statut.includes('CLOTUR')) return 'CLOTURE'
    return statut
  }, [objet?.statut])

  // Fonction pour ouvrir le modal de mise à jour de statut
  const handleOpenUpdateModal = (type: 'retrouve' | 'cloture') => {
    setActionType(type)
    ...
    setDateRetrouve(today)
    setUpdateError(null)
    setIsUpdateStatutModalOpen(true)
  }

  // Fonction pour mettre à jour le statut (version statique)
  const handleUpdateStatut = async () => {
    ...
    try {
      ...
      let dateRetrouveParam: string | undefined

      if (actionType === 'retrouve') {
        statutValue = 'RETROUVÉ'
        // Utiliser maxDate si disponible, sinon calculer côté client uniquement
        const today = typeof window !== 'undefined' ? new Date().toISOString().split('T')[0] : maxDate || ''
        dateRetrouveParam = dateRetrouve || today
      } else {
        statutValue = 'CLÔTURÉ'
      }
      ...
      if (dateRetrouveParam) {
        queryParams.append('dateRetrouve', dateRetrouveParam)
      }

      await api.patch(
        `/objets-perdus/${objet.id}/statut?${queryParams.toString()}`
      )

      await refetch()
      
      // Afficher un message de succès
      alert(`Statut mis à jour : ${statutValue}`)
  ...
  const canUpdateStatut = useMemo(() => {
    return normalizedStatut === 'EN_RECHERCHE' || normalizedStatut === 'RETROUVE'
  }, [normalizedStatut])
      ...
      <div className="min-h-screen flex items-center justify-center">
        <Card className="max-w-md">
          <CardBody className="p-6 text-center">
            ...
            <p className="text-slate-600 mb-4">{error || 'Objet perdu non trouvé'}</p>
            <Button onClick={() => router.back()} className="bg-orange-600 hover:bg-orange-700 text-white">
  ...
  const dateDeclarationFormatee =  formatDateTime(objet.dateDeclaration)
  const datePerteFormatee = objet.datePerteFormatee || formatDate(objet.datePerte)
    ...
    <div className="min-h-screen space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-600">
                {objet.isContainer && objet.containerDetails 
                  ? `${containerTypeLabels[objet.containerDetails.type] || objet.containerDetails.type} - ${objet.typeObjet}`
                  : objet.typeObjet
                }
              </p>
            </div>
      ...
      <div className="flex items-center gap-3">
        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold border ${getStatutColor(objet.statut)}`}>
          {getStatutLabel(objet.statut) === 'EN RECHERCHE' && <AlertTriangle className="w-4 h-4" />}
          {getStatutLabel(objet.statut) === 'RETROUVÉ' && <CheckCircle className="w-4 h-4" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {objet.isContainer && objet.containerDetails ? (
            <>
              ...
              <Card className="bg-white border border-gray-200">
                <CardBody className="p-6">
                  <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                    {React.createElement(containerTypeIcons[objet.containerDetails.type] || ShoppingBag, { 
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <p className="font-semibold text-slate-900">
                        {containerTypeLabels[objet.containerDetails.type] || objet.containerDetails.type}
                      </p>
                    </div>
                    {objet.containerDetails.couleur && (
                      <div>
                        <p className="text-sm text-slate-500">Couleur</p>
                        <p className="font-semibold text-slate-900">{objet.containerDetails.couleur}</p>
                      </div>
                    )}
                    {objet.containerDetails.marque && (
                      <div>
                        <p className="text-sm text-slate-500">Marque</p>
                        <p className="font-semibold text-slate-900">{objet.containerDetails.marque}</p>
                      </div>
                    )}
                    {objet.containerDetails.taille && (
                      <div>
                        <p className="text-sm text-slate-500">Taille</p>
                        <p className="font-semibold text-slate-900">{objet.containerDetails.taille}</p>
                      </div>
                    )}
                    {objet.valeurEstimee && (
                      <div>
                  ...
                  {objet.containerDetails.signesDistinctifs && (
                    <div className="mb-4">
              ...
              {objet.containerDetails.inventory && objet.containerDetails.inventory.length > 0 && (
                <Card className="bg-white border border-gray-200">
                  <CardBody className="p-6">
                    <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                      <Layers className="w-5 h-5 text-purple-600" />
                      Inventaire du contenant ({objet.containerDetails.inventory.length} objets)
                    </h2>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {objet.containerDetails.inventory.map((item: any, index: number) => {
          ...
          <Card className="bg-white border border-gray-200">
            <CardBody className="p-6">
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                ...
                Lieu et circonstances de la perte
              </h2>
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <p className="text-sm text-slate-500">Lieu</p>
                  <p className="font-semibold text-slate-900">{objet.lieuPerte}</p>
                </div>
                {objet.adresseLieu && (
                  <div className="col-span-2">
                ...
                <div>
                  <p className="text-sm text-slate-500">Date de perte</p>
                  <p className="font-semibold text-slate-900">{datePerteFormatee}</p>
                </div>
                {objet.heurePerte && (
                  <div>
                    <p className="text-sm text-slate-500">Heure approximative</p>
                    <p className="font-semibold text-slate-900">{objet.heurePerte}</p>
                  </div>
                )}
              </div>
            </CardBody>
          ...
          <Card className="bg-white border border-gray-200">
            <CardBody className="p-6">
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                ...
                Objets retrouvés correspondants
              </h2>
              <p className="text-sm text-slate-500 text-center py-4">Aucun objet retrouvé correspondant trouvé</p>
            </CardBody>
          </Card>
        </div>
        ...
        <div className="space-y-6">
            ...
            <CardBody className="p-6">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="font-semibold text-slate-900">
                    {objet.declarant?.prenom || ''} {objet.declarant?.nom || ''}
                  </p>
                </div>
                {objet.declarant?.telephone && (
                  <div className="flex items-center gap-2">
                    <Phone className="w-4 h-4 text-slate-400" />
                    <p className="text-slate-700">{objet.declarant.telephone}</p>
                  </div>
                )}
                {objet.declarant?.email && (
                  <div className="flex items-center gap-2">
                    <Mail className="w-4 h-4 text-slate-400" />
                    <p className="text-slate-700">{objet.declarant.email}</p>
                  </div>
                )}
                {objet.declarant?.adresse && (
                  <div>
                    <p className="text-sm text-slate-500">Adresse</p>
                    <p className="text-slate-700">{objet.declarant.adresse}</p>
                  </div>
                )}
                {objet.declarant?.cni && (
                  <div>
                    <p className="text-sm text-slate-500">N° CNI</p>
                    <p className="text-slate-700">{objet.declarant.cni}</p>
                  </div>
                )}
              </div>
            </CardBody>
            ...
            <Card className="bg-white border border-gray-200">
              <CardBody className="p-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-slate-500">{objet.agent.matricule}</p>
                </div>
              </CardBody>
            </Card>
          )}
            ...
            <CardBody className="p-6">
              ...
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <p className="font-semibold text-slate-900">{dateDeclarationFormatee}</p>
                </div>
                {objet.dateRetrouveFormatee && (
                  <div>
                    <p className="text-sm text-slate-500">Date de retrouvaille</p>
                    <p className="font-semibold text-green-600">{objet.dateRetrouveFormatee}</p>
                  </div>
                )}
              </div>
            </CardBody>
          ...
          {canUpdateStatut && (
            <Card className="bg-white border border-gray-200">
              <CardBody className="p-6">
                ...
                <div className="space-y-3">
                  {normalizedStatut === 'EN_RECHERCHE' && (
                    <>
                      <Button
                        onClick={() => handleOpenUpdateModal('retrouve')}
                      ...
                      <Button
                        onClick={() => handleOpenUpdateModal('cloture')}
          ...
          {normalizedStatut === 'CLOTURE' && (
            <Card className="bg-white border border-gray-200">
              <CardBody className="p-6">
                <div className="flex items-center gap-3 text-slate-600">
                  <CheckCircle className="w-5 h-5 text-green-600" />
                  <p className="font-medium">Processus terminé - Dossier clôturé</p>
                </div>
              </CardBody>
            </Card>
          )}
      ...
      <Modal isOpen={isUpdateStatutModalOpen} onClose={() => setIsUpdateStatutModalOpen(false)}>
        <ModalHeader>
          <ModalTitle>
            {actionType === 'retrouve' ? 'Marquer comme retrouvé' : 'Clôturer la déclaration'}
          </ModalTitle>
        ...
        <ModalBody>
          <div className="space-y-4">
            {actionType === 'retrouve' && (
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  Date de retrouvaille <span className="text-red-500">*</span>
                </label>
                <Input
                  type="date"
                  value={dateRetrouve}
                  onChange={(e) => setDateRetrouve(e.target.value)}
                  inputSize="md"
                  max={maxDate}
                  required
                />
            ...
            {actionType === 'cloture' && normalizedStatut === 'EN_RECHERCHE' && (
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-800">
                      Vous êtes sur le point de clôturer directement cette déclaration sans marquer l'objet comme retrouvé. 
            ...
            {actionType === 'cloture' && normalizedStatut === 'RETROUVE' && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start gap-2">
                  <CheckCircle className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="font-medium text-blue-900 mb-1">Clôture du dossier</p>
                    <p className="text-sm text-blue-800">
                      Vous êtes sur le point de clôturer cette déclaration. L'objet a déjà été marqué comme retrouvé.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <ModalFooter>
          ...
          <Button
            ...
            disabled={actionType === 'retrouve' && !dateRetrouve}
          >
            <CheckCircle className="w-4 h-4" />
            Confirmer
          </Button>
