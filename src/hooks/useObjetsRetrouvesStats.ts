...
export interface ObjetsRetrouvesStatsResponse {
  stats: ObjetRetrouveStats
  topTypes: TopType[]
...
export const useObjetsRetrouvesStats = () => {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      console.log('📊 Chargement des stats objets retrouvés:', {
        commissariatId,
        periode: periodeToSend,
        dateDebut: dateDebut || dates.dateDebut,
        dateFin: dateFin || dates.dateFin
      ...
      const response = await api.get(`/objets-retrouves/dashboard?${params.toString()}`)

      console.log('✅ Stats objets retrouvés récupérées:', response.data)
