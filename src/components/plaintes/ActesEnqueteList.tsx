...
export default function ActesEnqueteList({ plainteId }: { plainteId?: string }) {
  ...
  const handleAddActe = async () => {
    ...
    try {
      console.log('🔵 Envoi de l\'acte à l\'API...', { plainteId })
      ...
      console.log('📦 Données envoyées:', acteData)
      
      const response = await api.post(`/plaintes/${plainteId}/actes-enquete`, acteData)
      ...
      setShowModal(false)
      alert('✅ Acte d\'enquête ajouté avec succès !')
    } catch (error: any) {
      console.error('❌ Erreur ajout acte:', error)
      console.error('❌ Détails:', error.response?.data)
      alert(`Erreur lors de l'ajout de l'acte: ${error.response?.data?.error || error.message}`)
    }
  }
        ...
        <Card>
          <CardBody className="p-6">
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-slate-500 mt-1">Cliquez sur "Ajouter un acte" pour commencer</p>
            </div>
          </CardBody>
        </Card>
