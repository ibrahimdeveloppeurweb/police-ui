...
export default function PreuvesList({ plainteId }: { plainteId?: string }) {
  ...
  const handleAddPreuve = async () => {
    ...
    try {
      console.log('🔵 Envoi de la preuve à l\'API...', { plainteId })
      ...
      console.log('📦 Données envoyées:', preuveData)
      
      const response = await api.post(`/plaintes/${plainteId}/preuves`, preuveData)
      ...
      setShowModal(false)
      alert('✅ Preuve ajoutée avec succès !')
    } catch (error: any) {
      console.error('❌ Erreur ajout preuve:', error)
      console.error('❌ Détails:', error.response?.data)
      alert(`Erreur lors de l'ajout de la preuve: ${error.response?.data?.error || error.message}`)
    }
  }
        ...
        <Card>
          <CardBody className="p-6">
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-slate-500 mt-1">Cliquez sur "Ajouter une preuve" pour commencer</p>
            </div>
          </CardBody>
        </Card>
